...
function updateJobSection(job) {
    ...
    let start = timeParser(job.time_created);

    selectAndFill('date-created', start.date);
    selectAndFill('time-created', start.time);

    let end = timeParser(job.time_done);
    selectAndFill('date-done', end.date);
    selectAndFill('time-done', end.time);
