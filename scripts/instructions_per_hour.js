...
export async function instructionsPerHour(year, month, day) {
    ...
    let max = {
        x: d3.max(instructions_per_hour, d => d.hour),
        y: d3.max(instructions_per_hour, d => parseInt(d.total_quantity))
    }

    let domain = {
        x: instructions_per_hour.map(d => d.hour),
        y: [0, max.y]
    }
    ...
    svg.append('text')
        ...
        .attr('class', 'graph-label')
        .text('Total Instruction')

    g.selectAll('.bar')
        .data(instructions_per_hour)
        .enter()
        .append('rect')
        .attr('class', 'bar')
